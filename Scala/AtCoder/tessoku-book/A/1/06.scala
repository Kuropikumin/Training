object Main {
  def main(args: Array[String]): Unit = {
    ...
    val total_visitors_list = Array.fill[Int]( period )( 0 )

    val visitors_list = scala.io.StdIn.readLine.split( " " ).map{ _.toInt }
    ( 0 until period ).foreach{ i =>
      if( i == 0 ) total_visitors_list( i ) = visitors_list( i )
      else         total_visitors_list( i ) = visitors_list( i ) + total_visitors_list( i - 1 )
    }
