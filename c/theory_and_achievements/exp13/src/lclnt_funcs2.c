...
int validate_ticket()
{
    ...
    if(!have_ticket)  return(0); // チケットを持っていない場合

    sprintf(buf, "VALD %s", ticket_buf);    // チケットを持っている旨のメッセージを作成
    if ( (response = do_transaction(buf)) == NULL ) return(-1); // メッセージ処理に失敗した場合

    narrate("Validated ticket: ", response);

    if ( strncmp(response, "GOOD", 4) == 0 ) return(0);
